...
describe('PaymentChannel', () => {
    ...
    it.skip('should require sequential seqno', async () => {
         // IMHO jumping over seqno's shouldn't be allowed, since
         // it means that some state in between was not commited
         // Or is it?

        let dataBefore = await tonChannel.getChannelData();
        const {sentA, sentB} = calcSends(dataBefore);

        const withdrawA = dataBefore.balance.balanceA / BigInt(getRandomInt(10, 100));
        const withdrawB = dataBefore.balance.balanceB / BigInt(getRandomInt(10, 100));


        const testCommit: BalanceCommit = {
            withdrawA: dataBefore.balance.withdrawA + withdrawA,
            withdrawB: dataBefore.balance.withdrawB + withdrawB,
            seqnoA: dataBefore.seqnoA + BigInt(getRandomInt(2, 1000)),
            seqnoB: dataBefore.seqnoB + BigInt(getRandomInt(2, 1000)),
            sentA,
            sentB
        };
        const commitBody = PaymentChannel.cooperativeCommitBody(testCommit, tonChannelConfig.id);
    ...
    it.skip('if balance changes in after commit is signed, commit should not be accepted', async () => {
        // Should it
        let dataBefore = await tonChannel.getChannelData();
        const {sentA, sentB} = calcSends(dataBefore);
        ...
        // Now the unexpected topUp
        await tonChannel.sendTopUp(walletB.getSender(), false, toNano('1'));
        ...
        expect(res.transactions).toHaveTransaction({
            ...
            from: walletA.address,
            aborted: true,
            exitCode: Errors.ERROR_INCORRECT_SUMMARIZED_BALANCE
        });
    });
